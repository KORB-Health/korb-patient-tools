...
12. ~~The pricing table layout is not finished.~~ **DONE 2026-09-15.** Closed by
   Don's own diagnosis, and it was never the geometry. Four passes had chased
   column widths while the tables "ran off onto the background" because
   `.grid td` and `.kv td` carry NO background: an odd row was not a pale cell
   on a page, it WAS the page, and only even rows had a colour. Against the old
   #FBFAF6 that edge was invisible, so the fault read as a font or spacing
   problem. The page is now brand cream #ECE9D1 and every table cell has an
   explicit white surface, screen only - the PDF stays white. Verified live:
   page #ECE9D1, rows #FFFFFF/#F7F7F4, no row matching the page.
   **The lesson is the diagnosis, not the fix.** Four passes of measuring
   geometry could not find a colour problem. When something "looks off" and the
   measurements keep coming back correct, the measurement is of the wrong thing.
